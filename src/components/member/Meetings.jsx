...
export default function MemberMeetings() {
  ...
  useEffect(() => {
    ...
    apiFetch(`/member/meetings?filter=${tf}`)
    ...
    <>
      ...
      <div className="flex w-full md:w-auto gap-4 mb-8">
        <div className="relative flex-1 md:w-96">
          ...
          <input
            className="w-full pl-12 pr-4 py-3 bg-surface-container-lowest border border-outline-variant/20 rounded-xl text-sm focus:outline-none focus:border-primary"
        ...
        {loading ? (
          ...
          <div className="px-6 py-16 text-center">
            ...
            {(error || timeFilter !== 'All Time' || searchQ) && (
              <button 
                onClick={() => { setError(null); setTimeFilter('All Time'); setSearchQ(''); }}
            ...
            <tbody>
              {filtered.map((m, idx) => {
                ...
                const displayDate = m.meetingDate
                  ...
                  : '—';
                return (
                  <tr key={m._id || idx} className="cursor-pointer group" onClick={() => navigate(`/member/meeting-detail?id=${m._id}`)}>
                    <td>
                    ...
                    <td className="text-right relative">
                      ...
                      <div className={`ts-dropdown ${openDD === idx ? 'open' : ''}`}>
                        <button className="ts-dropdown-item" onClick={e => { e.stopPropagation(); navigate(`/member/meeting-detail?id=${m._id}`); setOpenDD(null); }}><span className="material-symbols-outlined text-sm">visibility</span>View Details</button>
                        {m.momId && (
                          <button className="ts-dropdown-item" onClick={e => { e.stopPropagation(); navigate(`/member/mom-detail?id=${m.momId}`); setOpenDD(null); }}><span className="material-symbols-outlined text-sm">description</span>View MOM</button>
                        )}
                      </div>
                    </td>
                  </tr>
