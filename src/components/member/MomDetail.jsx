...
const DIALOGUES = [
  {
    id: 1,
    author: 'Elena Rossi',
    text: '"Should we consider a darker mode for the Atelier concept, or keep it strictly paper-light?"',
    accepted: null,
  },
  {
    id: 2,
    author: 'Julian Pierce',
    text: '"Let\'s focus on the light foundation first. The \'warmth\' is our differentiator."',
    accepted: true,
  },
];
...
export default function MemberMomDetail() {
  ...
  function handlePost() {
    if (dialogueText.trim()) {
      setDialogues(prev => [...prev, { id: Date.now(), author: 'You', text: `"${dialogueText.trim()}"`, accepted: null }]);
      setDialogueText('');
    }
  }

  function toggleAccepted(id, value) {
    setDialogues(prev => prev.map(d => d.id === id ? { ...d, accepted: d.accepted === value ? null : value } : d));
  }
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-14">
        ...
        <button className="btn-secondary gap-2 flex-shrink-0" onClick={() => navigate('/member/meeting-detail')}>
        ...
        <div className="col-span-12 lg:col-span-4 space-y-10">
          <section>
            ...
            <div className="ts-card p-5 space-y-4">
              {[{name:'Julian Pierce',role:'Design Lead'},{name:'Sarah Kim',role:'Product Manager'},{name:"Liam O'Brien",role:'CTO'},{name:'Elena Rossi',role:'Brand Strategist'}].map((p,i)=>(
          ...
          {/* Dialogue with Accepted/Not Accepted indicators */}
          <section>
            <h3 className="font-headline text-xl text-on-surface mb-5 italic">Dialogue</h3>
            <div className="space-y-5">
              {dialogues.map(d => (
                <div key={d.id} className="relative pl-5 before:absolute before:left-0 before:top-0 before:bottom-0 before:w-px before:bg-outline-variant/30">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-xs font-mono text-outline uppercase">{d.author}</p>
                    {/* Accepted / Not Accepted tag */}
                    {d.accepted === true && (
                  ...
                  <p className="text-sm text-on-surface-variant italic leading-relaxed">{d.text}</p>
                </div>
              ))}

              {/* Member dialogue input */}
