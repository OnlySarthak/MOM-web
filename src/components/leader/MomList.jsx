...
export default function LeaderMomList() {
  ...
  const moms = [
    ...
    { project: 'Studio Ops', projectColor: 'text-primary', date: 'Oct 18, 2023', title: 'Weekly Resource Allocation & Burn Rate', desc: 'Reviewing team capacity for the upcoming sprint. Shifted focus from maintenance tasks to the high-end UI overhaul.', decisions: 3, actions: 8, members: [{ initials: 'JT', color: 'bg-primary' }, { initials: 'EV', color: 'bg-secondary' }], extra: 0 },
  ];
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8 pb-6 border-b border-outline-variant/10">
        <div className="flex items-center gap-4">
          <button className="flex items-center gap-2 px-4 py-2 bg-surface-container-lowest border border-outline-variant/15 rounded-xl text-sm font-medium hover:bg-surface-container-low transition-colors">
            <span className="material-symbols-outlined text-outline">filter_list</span>All Participants<span className="material-symbols-outlined text-outline">expand_more</span>
          </button>
          <button className="flex items-center gap-2 px-4 py-2 bg-surface-container-lowest border border-outline-variant/15 rounded-xl text-sm font-medium hover:bg-surface-container-low transition-colors">
        ...
        <div className="flex items-center gap-2 text-xs font-mono text-outline uppercase tracking-tighter">
          ...
          {moms.length} MOMs in your team
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
        {moms.map((m, i) => (
          <div key={i} className="ts-card p-6 hover:-translate-y-1 transition-all duration-300 flex flex-col group cursor-pointer" onClick={() => navigate('/leader/mom-detail')}>
            ...
            <div className="mt-auto space-y-4">
              <div className="flex gap-2">
                <span className="flex items-center gap-1.5 px-2.5 py-1 bg-primary-fixed/30 rounded-full text-[10px] font-bold text-primary uppercase"><span className="w-1.5 h-1.5 rounded-full bg-primary"></span>{m.decisions} Decisions</span>
                <span className="flex items-center gap-1.5 px-2.5 py-1 bg-surface-container-high rounded-full text-[10px] font-bold text-on-surface-variant uppercase"><span className="material-symbols-outlined text-xs">assignment</span>{m.actions} Actions</span>
              </div>
