...
export default function LeaderMomList() {
  ...
  useEffect(() => {
    ...
    apiFetch(`/leader/moms?filter=${tf}`)
      .then(res => {
        // Response is array directly (leader controller returns res.json(momsWithAttendees))
        setMoms(Array.isArray(res) ? res : (res.data || []));
      })
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8 pb-6 border-b border-outline-variant/10">
        <div className="flex items-center gap-4">
          <div className="relative">
            {/* Time filter wired to API — All Time and Earlier = current month (backend logic) */}
            <select
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
          {moms.map((mom, i) => {
            ...
            const displayDate = mom.createdAt ? new Date(mom.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—';
            return (
              <div
                key={mom._id || i}
                className="ts-card p-6 hover:-translate-y-1 transition-all duration-300 flex flex-col group cursor-pointer"
                onClick={() => navigate(`/leader/mom-detail?id=${mom._id}`)}
              >
                <div className="flex justify-between items-start mb-4">
                  {/* label = contextLable (backend typo preserved) */}
                ...
                <div className="mt-auto space-y-4">
                  <div className="flex gap-2">
                    ...
                    {/* actions = totalTasks from backend */}
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-outline-variant/10">
                    <div className="flex -space-x-2">
                      {attendees.slice(0, 3).map((a, ai) => (
                        <div key={ai} className="w-7 h-7 rounded-full bg-primary text-white text-[8px] font-bold flex items-center justify-center ring-2 ring-white">
